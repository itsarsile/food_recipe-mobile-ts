...
import { useDeleteSavedRecipesMutation, useGetSavedRecipesQuery } from "@/src/features/save/saveApiSlice"
...
export default function SavedRecipe() {
  ...
  console.log("🚀 ~ file: saved.tsx:18 ~ SavedRecipe ~ user:", user.id)
  const { data, isLoading, refetch } = useGetSavedRecipesQuery(user.id)
  const [deleteSavedRecipes, {isLoading:isDeleting}] = useDeleteSavedRecipesMutation()
  useFocusEffect(
    React.useCallback(() => {
      refetch()
    }, [refetch])
        ...
        [
          ...
          },
          {
            ...
            onPress: async () => {
              ...
              await deleteSavedRecipes(saveId);
              refetch();
      ...
      <View key={recipe.recipeId} className="flex-row justify-between mb-5 ">
        ...
        <View className="gap-0">
          <IconButton
            ...
            size={24}
            iconColor="red"
            onPress={() => handleDelete(recipe.savedId)}
          />
        </View>
      </View>
    ));
